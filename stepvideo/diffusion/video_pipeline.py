...
def call_api_gen(url, api, port=8080):
    ...
    async def _fn(samples, *args, **kwargs):
        ...
        async with aiohttp.ClientSession() as sess:
            ...
            try:
                # Print the first 200 bytes of the raw response for inspection.
                print(f"[DEBUG] Raw response before deserialization: {result[:200]}")
                ...
                try:
                    ...
                    # Uncomment below if you want to convert the JSON response to pickle format.
                    pickled_data = pickle.dumps(response_json)
                    response_data=pickled_data
                    print(f"[DEBUG] Converted JSON to pickle format, length: {len(pickled_data)} bytes")
...
class StepVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        Args:
            ...
            output_type (`str`, *optional*, defaults to `"mp4"`):
                The output format of the generated image. Choose between `PIL.Image`, `np.array`, or `"latent"`.
